...
- `*` - позволяет подставить во фразу любые пропущенные слова
...
- `after:YYYY-MM-DD` и `before:YYYY-MM-DD` - выдает результаты созданные `после`/`до` даты
...
- Используйте правильный поиск (google, advanced search, scholar, travel).
